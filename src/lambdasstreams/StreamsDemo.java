...
public class StreamsDemo {
	...
	public String joinStream() {
		return Stream.of("this", "is", "a", "stream", "of", "strings").collect(Collectors.joining(" "));
	}
	...
	public double sumFirstNBigDecimals(int num) {
		return Stream.iterate(BigDecimal.ONE, val -> val.add(BigDecimal.ONE)).limit(num).mapToDouble(BigDecimal::doubleValue).sum();
	}
	
	public Double sumRandoms1(int num) {
		return Stream.generate(Math::random).limit(num).reduce(Double::sum).orElse(0.0);
	}
	
	public Double sumRandoms2(int num) {
		return Stream.generate(Math::random).limit(num).reduce((acc, n) -> {
	...
	public Double sumRandoms3(int num) {
		Random r = new Random();
		return r.doubles().limit(num).sum();
	}
